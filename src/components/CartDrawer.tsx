...
const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  // Delay render slightly (Safari safety)
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Drawer */}
      <div className="absolute inset-y-0 right-0 w-full max-w-md bg-white flex flex-col h-full shadow-xl">
        {/* Header */}
          ...
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <p className="text-stone-500 italic">
                Your basket is empty. Let’s find some fresh produce.
              </p>
              <button
                onClick={onClose}
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-stone-200 bg-stone-50 space-y-4">
            ...
            <Link
              to="/checkout"
              onClick={onClose}
              className="block w-full text-center py-4 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700"
            >
              Proceed to Checkout
            </Link>
